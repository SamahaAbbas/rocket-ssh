...
getAppVersion(){
    version=$(sudo curl -Ls "https://api.github.com/repos/SamahaAbbas/rocket-ssh/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    echo $version;
}
...
copyPanelRepo(){
   ...
   link=https://github.com/SamahaAbbas/rocket-ssh/raw/master/app.zip
...
installationInfo(){
    clear
    echo -e "\n"
    bannerText=$(curl -s https://raw.githubusercontent.com/SamahaAbbas/rocket-ssh/master/rocket-banner.txt)
    printf "%s" "$bannerText"
    echo -e "\n"
...
nethogsLink=https://raw.githubusercontent.com/SamahaAbbas/nethogs-json/master/install.sh
